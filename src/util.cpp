...
const char * const PBCCHAIN_CONF_FILENAME = "pbcchain.conf";
const char * const PBCCHAIN_PID_FILENAME = "pbcchaind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "pbcchaind -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Pbcchain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Pbcchain
    // Mac: ~/Library/Application Support/Pbcchain
    // Unix: ~/.pbcchain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Pbcchain";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Pbcchain";
#else
    // Unix
    return pathRet / ".pbcchain";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", PBCCHAIN_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", PBCCHAIN_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Pbcchain Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Pbcchain Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Pbcchain Core developers";
    }
