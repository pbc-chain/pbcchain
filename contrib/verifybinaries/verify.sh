#!/usr/bin/env bash
# Copyright (c) 2016 The Pbcchain Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   pbcchaincore.org and pbcchain.org and compares them.
...
WORKINGDIR="/tmp/pbcchain_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://pbcchaincore.org"
HOST2="https://pbcchain.org"
BASEDIR="/bin/"
VERSIONPREFIX="pbcchain-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'pbcchain-',
...
if [ $? -ne 0 ]; then
   echo "pbcchain.org failed to provide signature file, but pbcchaincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "pbcchain.org and pbcchaincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Pbcchain Core binary release signing key installed?"
   fi
